...
exit={} - по-умолч. Словарь выходов-"сливов" на границах острова.
...
Действия:
1. Проверка клеток в четырех направлениях от исходной (формирование списков d, river, exit).
...
'''
def flow(i,j,point,d,mas,river=[],exit={},path=[]):
    path.append((i,j))
    if (i,j) not in river:
        river.append((i,j))
# 1.Проверка соседних клеток:
    if j+1 == (len(mas[i])-1): # Справа
        exit[(i,j+1)] = mas[i][j+1]
        d.append((i,j+1))
    else:
...
# Рекурсивный вызов:
    if len(river)>1:
        for x,y in river:
            if (x,y) not in path:
                flow(x, y, point, d, mas, river, exit)
# Поднятие уровня рек:
    if exit!={}:
        for x,y in river:
            if min(exit.values())>mas[x][y]:
                mas[x][y] = min(exit.values())
                if (x,y) not in d:
                    d.append((x,y))
    river.clear()
    exit.clear()
